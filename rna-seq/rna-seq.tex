...
\section{Visualising the Cufflinks Analysis}
...
\begin{information}
\texttt{CummeRbund} takes the cuffdiff output and populates a SQLite database with varioys type of output generated by cuffdiff e.g, genes, transcripts, transcription start site, isoforms and CDS regions. The data from this database can be accessed and processed easily. This package comes with a number of in-built ploting functions that are commonly used for visualising the expression data. We strongly recommend reading through the bioconductor manual and user guide of CummeRbund to learn about various functionality of the tool. The reference is provided in the resource section.
\end{information}
...
\texttt{0.02} and the BCV is the square root of the common dispersion \texttt{(/sqrt0.02 = 0.14)}.
...
in the second column, the the logCP, the P-Value and the adjusted P-Value. The ensemble
...
We then use the function \texttt{getBM} to get the gene symbol data we want (this can
taken about a minute or so):
\begin{lstlisting}
...
\begin{lstlisting}
write.csv(as.data.frame(de), file='DE_genes.csv')
\end{lstlisting}
\end{steps}
